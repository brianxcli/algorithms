...
public abstract class IGUI {
	...
	/**
	 * Is called when the mouse is dragged, and is passed the 
	 * MouseEvent object for that drag.
	 */
	protected abstract void onMouseDragged(Vector vector);
	
	/**
	 * Is called when the mouse wheel is moved, and is passed the 
	 * MouseEvent object for that mouse wheel move.
	 */
	...
	private static final int DEFAULT_DRAWING_HEIGHT = 450;
	private static final int DEFAULT_DRAWING_WIDTH = 742;
	...
	private void initialise() {
		...
		drawing.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent e) {
				int dragX = dragEvent.getX();
				int dragY = dragEvent.getY();
				int x = e.getX();
				int y = e.getY();
				...
				if (dragX == x && dragY == y) {
					...
					onMouseDragged(new Vector(x - dragX, y - dragY));
				}
